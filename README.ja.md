...
モデル・デバイス・閾値・言語・アンダースコア設定・不要タグ一覧などは終了時に `settings.xml` へ自動保存されます。
...
| プリセット | 対象 | VRAM目安 |
|---|---|---|
| Anime SDXL | イラスト・アニメ系 | 約16GB |
| Photo SDXL | 実写写真系 | 約16GB |

両プリセットとも `gradient_checkpointing=true` に設定しており、VRAM 16GB以内で学習できる・・・と思われます。
（自分の環境がVRAM 12GBなので、最低限の設定しか試していません。）
